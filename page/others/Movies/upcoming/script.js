...
function buildFilterURL(page) {
  ...
  const isUpcomingPage = pageTitle.toLowerCase().includes("upcoming");

  if (isUpcomingPage) {
    const today = new Date().toISOString().split("T")[0];
    baseUrl += `&primary_release_date.gte=${dateFromInput && dateFromInput.value ? dateFromInput.value : today}`;

    const sixMonthsLater = new Date(Date.now() + 180 * 24 * 60 * 60 * 1000)
      .toISOString()
      .split("T")[0];
    baseUrl += `&primary_release_date.lte=${dateToInput && dateToInput.value ? dateToInput.value : sixMonthsLater}`;

    baseUrl += `&with_release_type=1|2`;
  } else {
